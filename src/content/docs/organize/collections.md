...
When there's a clear relationship, it's easy to bring Objects together using a [Query](/organize/queries/), such as 'everything tagged inspiration'. But when there isn't a clear [Property](/organize/properties/) or [Type](/organize/types/) that can link everything together, this is when Collections work best. It's a curated container for everything you see having a relationship together but the system can't easily define.

## When to use Collections

The easiest way to understand Collections is to compare them with [Queries](/organize/queries/). With Collections, you're curating a group of Objects that don't change much over time. With Queries, you're filtering Objects that likely change over time.
...
Collections live in all the same places as [Types](/organize/types/)—the Sidebar, the Create menu, and Channel Settings. And like Types, every Collection has Views that work in the same way.
...
To learn more about how to manage Collections and their settings, see the [Views](/organize/views/#how-it-works) section.
...
* Right click the Object in a [View](/organize/views/) or open the 'three dots' menu in the top right corner.
...
Use the [bulk-editing feature in Views](/organize/views/#bulk-editing-objects) to organize many Objects at the same time in a Collection. You can create a Query to find the Objects you're looking for, and then add them all to a Collection in bulk.

#### To set a default Type

When adding a new Object into a Collection, you can choose the default [Type](/organize/types/) and [Template](/organize/templates/) used. When viewing a Collection, click on the dropdown button next to 'New' to change the default.
...
Edits made to the View of an Inline Collection apply only to that specific block. Editing an Inline View inside an Object doesn't affect the master View — that stays intact at the [Type](/organize/types/), [Query](/organize/queries/), or [Collection](/organize/collections/) level. This means each Object can have its own version of the Inline View without ever touching the master.
...
You can add [Collections to your Sidebar as a Widget](/organize/views/#views-in-sidebar), just like any other View. However, instead of using Collections to simulate a [hierarchical folder-like structure in your Sidebar](/basics/sidebar/widgets/#layout-options), you can create a Page with link blocks:
